...
def main(args, extras) -> None:
    ...
    lora_config = LoraConfig(
        r=16,
        lora_alpha=32,
        target_modules=["c_qkv", "c_proj", "c_q", "c_kv"],
        lora_dropout=0.05,
        bias="none",
    )

    # Apply LoRA to the denoiser model
    system.denoiser_model = get_peft_model(system.denoiser_model, lora_config)
    ...
    if args.train:
        ...
        callbacks += [ProgressCallback(save_path=os.path.join(cfg.trial_dir, "progress"))] if args.gradio else [CustomProgressBar(refresh_rate=1)]
